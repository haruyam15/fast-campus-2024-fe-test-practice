  ...
  return (
    <div style={{ marginBottom: '200px' }}>
      <div style={{ marginBottom: '20px', fontFamily: 'Arial, sans-serif' }}>
        <h2 style={{ marginBottom: '10px' }}>정렬 기준: {sortBy}</h2>
        ...
        <div>
          <h2 style={{ marginBottom: '10px' }}>정렬 방향: {sortDirection}</h2>
          <div>
            {(
              Object.keys(SortDirection) as Array<keyof typeof SortDirection>
            ).map((sortType) => {
      ...
      {listData ? (
        <div>
          <table
            style={{ width: '720px', borderCollapse: 'collapse', fontSize: 14 }}
          >
            ...
            {SortProduct(listData, sortBy, sortDirection)?.map(
              (product: Product) => (
